...
if __name__ == "__main__":
    ...
    fragments = set(frag for mol in mols for frag, _ in find_fragments(mol))
    MolGraph.load_fragments(fragments=fragments)
    ...
    for i, s in enumerate(test_smiles):
        ...
        plt.figure()
        nx.draw(hmol.mol_tree, with_labels=True)
        plt.savefig(f'plt/nx_tree{i}.jpg')
        plt.close()

        plt.figure()
        nx.draw(hmol.mol_graph, with_labels=True)
        plt.savefig(f'plt/nx_graph{i}.jpg')
        plt.close()

    # Draw.MolsToGridImage([hmol.mol], molsPerRow=1, subImgSize=(500, 150)).save('0.jpg')
    # id_s = nx.get_node_attributes(hmol.mol_tree, 'label')
    # ss = []
    # for s in id_s:
    #     ss.extend([smi for smi in id_s[s]])
    #
    # Draw.MolsToGridImage([Chem.MolFromSmiles(s) for s in ss], subImgSize=(200, 150)).save('1.jpg')
